<?php
 ...
 /*   if(empty($_POST['nm']) || empty($_POST['cnm']) || empty($_POST['addr']) || empty($_POST['ph']) || empty($_POST['email']) || empty($_POST['profile']) || empty($_POST['pwd']))
    {
      $msg[]="one of your feild is empty";
    }
  */
	...
	$pwd=$_POST['pwd'];
        ...
        $q="insert into employers(er_fnm,er_pwd,er_company,er_add,er_ph,er_email,er_company_profile)
         values
        ('$nm','$pwd','$cnm','$addr','$ph','$email','$profile')";

         pg_query($conn,$q)or die("wrong query");
         pg_close($conn);
         header("location:employeerregister.php");
       }
 ?>
